...
generate-openai-seed:
	$(GRAPHFT) generate-openai-seed --corpus artifacts/corpus/types_v1.jsonl --out-dir artifacts --config examples/pipeline_config.yaml

build-dataset:
	$(GRAPHFT) build-dataset --corpus artifacts/corpus/types_v1.jsonl --openai-seed artifacts/openai/seed_pairs_v1.jsonl --schema-hash-file artifacts/metadata/schema_hash.json --out-dir artifacts --version 1 --config examples/pipeline_config.yaml
...
train-embedder:
	$(GRAPHFT) train-embedder --train artifacts/datasets/v1/train.jsonl --val artifacts/datasets/v1/val.jsonl --corpus artifacts/corpus/types_v1.jsonl --disable-lora --eval-every-epoch --benchmark-dir artifacts/datasets/v1/benchmarks --tracking-backend wandb --run-name qwen3-embedder-v1 --out-dir artifacts/models/qwen3-embedding-0.6b-ft

eval-retrieval:
	$(GRAPHFT) eval-retrieval --eval-set artifacts/datasets/v1/test.jsonl --corpus artifacts/corpus/types_v1.jsonl --base-model Qwen/Qwen3-Embedding-0.6B --tuned-model artifacts/models/qwen3-embedding-0.6b-ft --out-dir artifacts/eval

run-benchmark:
	$(GRAPHFT) run-benchmark --benchmark-dir artifacts/datasets/v1/benchmarks --corpus artifacts/corpus/types_v1.jsonl --model artifacts/models/qwen3-embedding-0.6b-ft --tracking-backend wandb --out-dir artifacts/eval/benchmarks
