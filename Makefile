...
TRAIN_SIZE ?= 10000
EPOCHS ?= 3
MODEL_OUT ?= artifacts/models/qwen3-v$(VERSION)-sdl
...
backfill-structural-corpus:
	$(GRAPHFT) backfill-structural-corpus --corpus artifacts/datasets/v1/corpus.jsonl --out artifacts/datasets/v1/corpus_structural.jsonl --worlds-dir artifacts/worlds/v1 --primary-retrieval-view sdl
...
train-embedder:
	$(GRAPHFT) train-embedder --train artifacts/datasets/v1/train.jsonl --val artifacts/datasets/v1/val.jsonl --corpus artifacts/datasets/v1/corpus.jsonl --disable-lora --positive-views typename,field_paths,sdl --primary-retrieval-view sdl --eval-every-epoch --benchmark-dir artifacts/datasets/v1/benchmarks --tracking-backend wandb --run-name qwen3-embedder-v1 --out-dir artifacts/models/qwen3-embedding-0.6b-ft

eval-retrieval:
	$(GRAPHFT) eval-retrieval --eval-set artifacts/datasets/v1/test.jsonl --corpus artifacts/datasets/v1/corpus.jsonl --base-model Qwen/Qwen3-Embedding-0.6B --tuned-model artifacts/models/qwen3-embedding-0.6b-ft --retrieval-view sdl --out-dir artifacts/eval

run-benchmark:
	$(GRAPHFT) run-benchmark --benchmark-dir artifacts/datasets/v1/benchmarks --corpus artifacts/datasets/v1/corpus.jsonl --model artifacts/models/qwen3-embedding-0.6b-ft --retrieval-view sdl --tracking-backend wandb --out-dir artifacts/eval/benchmarks
...
build-ann-index:
	$(GRAPHFT) build-ann-index --corpus artifacts/datasets/v1/corpus.jsonl --model artifacts/models/qwen3-embedding-0.6b-ft --retrieval-view sdl --out-dir artifacts/index
...
prod-run:
	...
	$(GRAPHFT) backfill-structural-corpus --corpus artifacts/datasets/v$(VERSION)/corpus.jsonl --out artifacts/datasets/v$(VERSION)/corpus_structural.jsonl --worlds-dir artifacts/worlds/v$(VERSION) --primary-retrieval-view sdl
	$(GRAPHFT) train-embedder --train artifacts/datasets/v$(VERSION)/train.jsonl --val artifacts/datasets/v$(VERSION)/val.jsonl --corpus artifacts/datasets/v$(VERSION)/corpus_structural.jsonl --model Qwen/Qwen3-Embedding-0.6B --positive-views sdl --primary-retrieval-view sdl --epochs $(EPOCHS) --disable-lora --tracking-backend wandb --eval-every-epoch --benchmark-dir artifacts/datasets/v$(VERSION)/benchmarks --out-dir $(MODEL_OUT)
	$(GRAPHFT) eval-retrieval --eval-set artifacts/datasets/v$(VERSION)/test.jsonl --corpus artifacts/datasets/v$(VERSION)/corpus_structural.jsonl --base-model Qwen/Qwen3-Embedding-0.6B --tuned-model $(MODEL_OUT) --retrieval-view sdl --out-dir $(EVAL_OUT)
	$(GRAPHFT) build-ann-index --corpus artifacts/datasets/v$(VERSION)/corpus_structural.jsonl --model $(MODEL_OUT) --retrieval-view sdl --out-dir $(INDEX_OUT)
